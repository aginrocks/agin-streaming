...
pub fn search_callback(name: &proc_macro2::Ident) -> proc_macro2::TokenStream {
    quote! {
...
pub fn link_resolver_callback(name: &proc_macro2::Ident) -> proc_macro2::TokenStream {
    quote! {
        ::plugin_sdk::handler::HandlerInfo::LinkResolver(::plugin_sdk::handler::LinkResolverHandler {
            callback: |ctx, request| Box::pin(async move { #name(ctx, request).await }),
        })
    }
}
